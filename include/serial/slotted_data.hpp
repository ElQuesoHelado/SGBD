...
struct SlottedDataHeader {
  ...
  SlottedDataHeader(std::span<unsigned char> &buffer)
      : free_bytes(*reinterpret_cast<uint16_t *>(buffer.data())),
        ...
        free_space_offset(*reinterpret_cast<uint16_t *>(buffer.data() + 4)) {
    // auto slot_data = buffer.subspan(6);
    buffer = buffer.subspan(6);

    slots.reserve(n_slots);

    for (uint16_t i = 0; i < n_slots; ++i) {
      slots.emplace_back(buffer);
    }
  }

  // Actualiza free_bytes tanto en page_header y slotted
  // @note Usa y recorta bytes de data para el slot
  size_t add_free_slot(PageHeader &page_header,
                       std::span<unsigned char> &data) {
    if (free_bytes < sizeof(Slot))
      throw std::runtime_error("No hay espacio para más slots");

    slots.emplace_back(data); // vacio
    n_slots++;
    free_bytes -= sizeof(Slot);
 ...
 */
inline size_t prepare_slotted_insert(SlottedDataHeader &header,
                                     size_t n_slot, size_t reg_size) {
 ...
 */
inline size_t prepare_slotted_update(SlottedDataHeader &header,
                                     size_t n_slot, size_t new_reg_size) {
  if (n_slot >= header.slots.size())
    throw std::runtime_error("Slot fuera de rango");
