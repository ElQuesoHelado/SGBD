...
std::vector<RegPtr> BPTree::search(Comparator &comp) {
  Comparator low = comp.get_low(),
             high = comp.get_high(),
             filter = comp.get_filter();

  auto root = load_node(root_id);

  std::println("Arbol con root en: {}", root_id);

  return search(root, low, high, filter);
}

std::vector<RegPtr> BPTree::search(BPNode &x,
                                   Comparator &low,
                                   Comparator &high,
                                   Comparator &filter) {
  ...
  if (x.is_leaf) {
    ...
    return linked_leaf_search(x, high, filter, i);
  } else {
    // One pass garantiza no ser utilizado otra vez
    auto child = load_node(x.ptrs[i]);

    unload_node(x);
    return search(child, low, high, filter);
  }
}
...
std::vector<RegPtr> BPTree::linked_leaf_search(
  ...
  if (i == x.n_keys && x.ptrs[i] != 0) {
    ...
    auto partial =
        linked_leaf_search(sibling, comp, filter, 0);
    reg_ptrs.insert(reg_ptrs.end(), partial.begin(),
                    partial.end());
  }
