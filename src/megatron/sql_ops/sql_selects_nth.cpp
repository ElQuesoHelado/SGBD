...
ResultSet Megatron::select_nth_reg(std::string &table_name, size_t nth) {
  ...
  return select_nth_reg(table_metadata, nth);
}

ResultSet Megatron::select_nth_reg(
  ...
  while (curr_page_id != disk_manager->NULL_BLOCK) {
    ...
    if (page_header.n_regs > nth) {
      result_set =
          select_nth_from_page(table_metadata, curr_page_id, nth);
...
ResultSet Megatron::select_nth_from_page(serial::TableMetadata &table_metadata,
  ...
  auto result_set =
      (table_metadata.are_regs_fixed)
          ...
          : select_nth_from_slotted_page(table_metadata, select_page_id, nth);
