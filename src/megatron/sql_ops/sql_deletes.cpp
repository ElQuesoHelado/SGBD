...
ResultSet Megatron::delete_condition(std::string &table_name, std::string &col_name, std::string &condition) {
  ...
  return delete_condition(table_metadata, col_name, condition);
}

ResultSet Megatron::delete_condition(serial::TableMetadata &table_metadata, std::string &col_name, std::string &condition) {
  ...
  while (curr_page_id != disk_manager->NULL_BLOCK) {
    ...
    auto partial_result_set = delete_from_page(table_metadata, curr_page_id, col_index, cond_val);
...
ResultSet Megatron::delete_from_page(serial::TableMetadata &table_metadata, size_t delete_page_id, size_t col_index, SQL_type &cond_val) {
  auto result_set =
      (table_metadata.are_regs_fixed)
          ...
          : delete_from_slotted_page(table_metadata, delete_page_id, col_index, cond_val);

  return result_set;
}

ResultSet Megatron::delete_from_fixed_page(serial::TableMetadata &table_metadata, size_t delete_page_id, size_t col_index, SQL_type &cond_val) {
...
ResultSet Megatron::delete_from_slotted_page(serial::TableMetadata &table_metadata, size_t delete_page_id, size_t col_index, SQL_type &cond_val) {
  auto &frame = buffer_manager->load_pin_page(delete_page_id);
  std::vector<unsigned char> &page_bytes = frame.page_bytes;
  auto page_bytes_it = page_bytes.begin();
